...
**Task** Check for problems that occurred when loading the Texas precinct results by running the following code. How many problems were there, as shown by the number of rows in the output table showing errors? What do you think the problem is that R is describing? Answer below. **Answer** It ran into a value that looked like a character string and didn't know what to do.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **Answer** The district data type is numerical data (dbl), the precinct data type is character (chr) data, and the mail data is numerical (dbl) data.
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **Answer**:402345
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **Answer** I think the inactive status is worth exploring, as well as why some of the confirmations were not returned or the legacy data.
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **Answer** (chr) character data
...
**Answer** Inconsistent capitalization, duplicates, inconsistent format of values.
...
**Answer** The function changed 1_linenumber to x1_linenumber.
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **Answer** The error says that it is an invalid type (character) of argument. This probably means we are using the wrong data type for the amount field as a character (chr) and need to change it to a number (dbl).
...
**Task** Run the following codeblock to attempt to change the datatype for the amount field to a number. What is the new data type (three letter code) for amount? Answer below. **Answer** (dbl) is the new data type.
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **Answer** There are 2 dupes for Derrick Hamilton.
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. Add a description of what this code does to your reference notebook. **Answer** 13 rows 
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **Answer** It got rid of any of the zip codes with more than 5 numbers.
...
**Task** Run the following codeblock to use str_tot_title() to standarize capitalization in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **Answer** Two misspellings remain, "Conowingoo" and "Conowing". 
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **Answer** One misspelling remains "Conowingoo".
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **Answer** None remain
