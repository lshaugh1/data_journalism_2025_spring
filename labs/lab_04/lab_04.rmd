...
cleaned_md_senate <- read_csv("data/cleaned_md_senate.csv")
...
**A1.** It looks like Alsobrooks largest spending categories are advertising, salaries, and list acquisition. Hogan's top spending expenditures are advertising, salaries, and fundraising consulting.

```{r}
cleaned_md_senate <- cleaned_md_senate |>
  mutate(purpose_clean = case_when(
    purpose_clean == "ADS" ~ "Media Advertising",  
    purpose_clean == "Online Advertising" ~ "Media Advertising",
    purpose_clean == "STAFFING - SEE MEMO" ~ "Salary",
    purpose_clean == "Media Production" ~ "Media Advertising",
    purpose_clean == "STAFFING SERVICES" ~ "Salary",
    purpose_clean == "Radio Advertising" ~ "Media Advertising",
    purpose_clean %in% c("Payroll Fee", "Payroll Fees", "Payroll Taxes", "Payroll Tax") ~ "Salary",
    TRUE ~ purpose_clean
  ))
...
**A2.** Both Alsobrooks' and Hogan's top spending habit was advertising. Salaries, meaning those paid for staff, was near the top of the list for both campaigns. Meanwhile, Alsobrooks spent a good amount on list acquisition which I found to be obtaining lists of voters to increase campaign awareness.
...
**A3.** I used ChatGpt for this question. It looks like Alsobrooks' spending for salaries increased as the election neared, which makes sense considering the increase in the amount of labor/work needed during the election year. The same can be said for Hogan. 

```{r}
media_senate <- cleaned_md_senate |>
   mutate(date = ymd(date)) |>
  filter(purpose_clean=="Salary") |>
  mutate(month=floor_date(date, "month")) |>
  group_by(candidate, month) |>
  summarise(
    total_spent_salaries = sum(amount, na.rm = TRUE)
  ) |>
  arrange(candidate, desc(total_spent_salaries))
...
**A4.** Alsobrooks takes the top spot for spending on salaries. It looks like Hogan only has one unique recipient, meanwhile Alsobrooks has over 20. 
...
**A5** I think I would take the angle of comparing the financial decisions for both campaigns, with Hogan technically having the upper hand of experience and funds but Alsobrooks ultimately winning the race. But more specifically, I think the difference in spending on staffing is super interesting since Alsobrooks spent nearly double than Hogan on salaries and won the race.
