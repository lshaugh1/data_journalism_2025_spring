...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **Answer** It contains geographic data for places in Prince George's County, MD. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The output is a map outlining the areas identified in the data above for PG County.
...
**Task** Run the following code. Describe the output in the space below. **Answer** It installed the tigris library. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** It looks like our dataframe above, but added a new geometry column. It also shows us all US counties, not just MD counties. The "STATEFP" column appears to contain a number for each state. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now, we see only MD counties. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now, we see the state of Maryland rendered with the counties outlined in black. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** Now we see PG County outlined and each precinct individually outlined. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** This code joined the precinct data with the voter turnout data. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The map is now colored to distinguish zip codes from each other, and shows a scale signifying the amount of total eligible voters in the precincts. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The dataframe consists of data for counties in Maryland and the amount of eligible voters divided up by party.
...
**Task** Run the following code. Describe the output in the space below. **Answer** The dataframe displays population data for each county.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We now have a new column, geometry, that contains the "multipolygon" data to draw an outline of the counties. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** The county name now eliminates the word "county" and only identifies counties by their official names, i.e. "Anne Arundel". 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We cleaned the population data names. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We joined the data frames for population and voters by county.
...
**Task** Run the following code. Describe the output in the space below. **Answer** We renamed county names like St. Mary's and Baltimore to conditionally mutate the NA values. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We calculated the number of voters per 10k and sorted from highest to lowest. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We built a chloropleth map with the color each of each county according to the number of voters per 10k on a color gradiant.  
...
**Task** Run the following code. Describe the output in the space below. **Answer** We applied the magma color scale to our map. 
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer** We changed the color scale to a "log" scale so we could see the differences more clearly. It appears that the Southeast regions had more voters per 10k. 
...
**Task** Run the following code. Describe the output in the space below. **Answer** We now have a column for Unaffiliated voters per 10k. 
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see, especially on the ends of the scale? **Answer** The Western region had the least amount of Unaffiliated voters, while the Northeast had the most. 
