...
**Answer** This code is simply pulling only the selected variables such as office_name, office_district, etc. to simplify our dataset. Then, the 'mutate' function is used to create a category, in this case it's percent_election_day, and tells it to do something. The thing it is doing is calculating a percentage for the percentage of votes cast on election day. 
...
**Answer** The results changed by arranging the percentage of election day votes in descending order. 
...
**Answer** This is an interesting answer because it shows us right away the counties with the least amount of voter turnout on election day, which is more interesting than the ones with the most voter turnout. It makes for a better story because then we can uncover different factors as to why there was such little voter turnout in these predominantly democratic counties, as opposed to the more republican counties.
...
**Answer** I asked ChatGPT for a few other options, here's what it came up with: str_to_lower(), which converts all characters in a string to lowercase; str_to_title(), which converts the first letter of each word to uppercase and the remaining letters to lowercase, making the string the title case; and str_trim() which removes leading and trailing white spaces from a string.
...
## **Answer** If we find an amount less than 100 in the amount column, then put "Less than $100" in the "amount_category" column. If there is an amount greater than or equal to 100 and less than 500, put "Between $100 and $499" in the "amount_category" column. If we find an amount greater than or equal to 500 and less than 1500, put "Between $500 and $1499" in the "amount_category column". If we find an amount greater than or equal to 1500 and less than 3000, put "Between $1500 and $2999" in the "amount_category" column. If we find an amount greater than or equal to 3000, put "$3,000 or more" in the "amount_category column". 
...
**Answer** That's important so that we aren't messing up the original table but making a new variable to select only those results that we desire from our selected code.
...
**Answer:**The expenditure_data column now puts the date into YYYY-MM-DD format.
...
**Answer** The data shows that the closer to election day you get, the more money is spent on campaigns. 
