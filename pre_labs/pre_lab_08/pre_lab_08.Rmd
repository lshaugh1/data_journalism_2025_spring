...
**Answer** The first line of code assigns the md_voters dataset to the same variable, md_voters. The pipe operator passes the md_voters into the mutate function, which we are asking to take the total difference in unaffiliated voters from 2022 to 2024 and arrange it in descending order. Then we are asking it to choose only the County, and UNAF_DIFF columns to give us a dataset that reflects only the county names and the values of the differences for 'UNAF_DIFF'. The head(10) function simply gives us the top 10 rows of the dataset. Finally, we are asking it to display the dataset by the name "maryland_top_counties". 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** A blank white square :)
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** No more blank white square! A chart with the x-axis named "County" that lists the names of the top 10 counties in Maryland with the biggest difference in unaffiliated voters, and the y-axis named "count" starting at 0 and counting up by intervals of 5000.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** From the last code block, the chart is now organized in ascending order from left to right, and the x-axis has been renamed to "reorder(County, UNAF_DIFF)". 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The chart flipped the x and y axises so that the bar chart is now horizontally ascending. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** The background is now just white, not gray. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last version of the chart? **Answer** The theme has change now so that the lines of the graph are just horizontal and white, and the background is a light blueish gray. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **Answer** We now have a title at the top and a source tag at the bottom, as well as the y-axis labeled as "net change".
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The dataset displays a date with the amount of total contributions to the right of it. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** We have a line graph displaying the amount of total contributions in intervals of 200 as our y-axis, and the date in terms of months on the x-axis. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** We have a specific date next to the month now. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. **Answer** The dates are slanted now for easier viewing. 
