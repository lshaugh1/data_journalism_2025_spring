...
**Task** Export the CSV file to your pre_lab_06 folder and read it into R by running the following codeblock. What problems do you see with the data? **Answer** There's no row for the headers and a ton of NAs or missing data. The office names are also mixed in with the candidates' names.
...
**Task** Get rid of rows we don't need by running the codeblock below. What problems remain? **Answer** Still confusing header names and the parties still need their own column as well as office names and precincts. 
...
**Task** Run the codeblock below. What changed, and what issues or problems remain? **Answer** Still some NAs, incorrect headers and invalid party names like "RLakeED'. 
