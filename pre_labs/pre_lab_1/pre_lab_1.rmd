...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **<chr>, <dbl>**
...
**Grouping the terms by term name and counting how many classes were offered during that term.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **We added the arrange() function to arrange the number of classes offered per term from largest to smallest, rather than it's default arrangement of smallest to largest.**
...
**The output changed by adding the department column to group by more than one column. This showed us the number of courses offered by each department in each term. By using the code arrange(term, department), we were able to sort by both department and term.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Compared to the previous codeblock, we used the sum(seats) function to add all of the values in the seats column to get the total number of seats offered in each term.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **We added the sum(seats), mean(seats), and median(seats) functions to calculate the total number of seats in a given term, the average number of seats in a given term, and the median number of seats in a given term. The top terms all end in the number '8'.**
...
**We added the min(seats) and max(seats) functions to get the highest and lowest number of seats for each department. The data is now sorted by the total number of classes in a term, the total number of seats, the mean/median of seats, and the min/max number of seats.**
...
1. Does the Communications department (the lowest number of seats) exhibit lower enrollment numbers than Business and Management?

2.  Are there more seats offered for virtual/hybrid classes compared to in-person classes?

3.  Why is there such a big difference between the median and mean for Chemistry seats?